...
import { addAssetFormCtx,useAssetFormCtx,DetailPanel,generateId,validateForm } from "./common-components";import { CustomAppLayout, Navigation } from "../commons/common-components";
import { useAuthorizedHeader, useAuthUserInfo } from "../commons/use-auth";
import {params_local_storage_key,BreadcrumbsDynmic} from "./common-components";
        ...
        return addAsset(headers, body)
          .then((data) => {
            ...
            setNotificationItems((item) => [
              ...item,
              {
                header: `Success to save`,
                type: "success",
                content: <Box>{'View:'}<Link href={`/asset_hub/${formData?.id}`}>{`${formData?.demo_name}`}</Link></Box>,
                dismissible: true,
...
//   const apigateway_endpoint = localStoredParams?.apigateway_endpoint;
  ...
  return (
    <CustomAppLayout
      ref={appLayout}
      navigation={<Navigation activeHref="/asset_hub" />}
